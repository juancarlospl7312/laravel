<script>
    ...
    jQuery(document).ready(function(){
        ...
        $.upload = function (file, ele) {
            ...
            out.append('_token', '<?php echo e(csrf_token()); ?>');
            out.append('file', file);

            $.ajax({
                url: '<?php echo e(action('Backend\DefaultController@uploadFileCKEditor')); ?>',
                method: "POST",
                data: out,
                cache: false,
                contentType: false,
                ...
                .done(function(response){
                    if(response !== undefined){
                        if(response.success === true){
                            let image = $('<img style="width: 100%;">').attr('src', "<?php echo e(asset('storage/')); ?>" + "/" + response.path);
        ...
        $.delete = function (src) {
            ...
            $.ajax({
                url: '<?php echo e(action('Backend\DefaultController@deleteFileCKEditor')); ?>',
                method: "POST",
                data: out,
                cache: false,
                contentType: false,
        ...
        form_action_cancel.on('click', function(){
            if(form_crud_edit.attr('id') === 'user-profile-form'){
                page_content.empty();
                page_content.load('<?php echo e(action('Backend\UserController@showProfile')); ?>');
            }
            else{
            ...
            .done(function(response){
                if(response !== undefined){
                    if(response.success === true){
                        if(form_crud_edit.attr('id') === 'user-profile-form'){
                            page_content.empty();
                            page_content.load('<?php echo e(action('Backend\UserController@editProfile')); ?>');
                        }
                        else{
...
</script><?php /**PATH D:\xampp\htdocs\workspace\laravel\resources\views/backend/includes/actions_form.blade.php ENDPATH**/ ?>